...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    ### to integer overflow.
    # outsideWordVector = N x D, centerWordVector = 1 x D

    sfmx = softmax(outsideVectors @ centerWordVec)  # shape = (N,)
    loss = -1 * np.log(sfmx[outsideWordIdx])  # loss is the log P(O==o|C==c)
    Uo = outsideVectors[outsideWordIdx, :]
    gradCenterVec = -Uo + sfmx @ outsideVectors
    gradOutsideVecs = sfmx[:, np.newaxis] @ centerWordVec[np.newaxis, :]
...
def negSamplingLossAndGradient(
    ...
    unisampleidx, unicount = np.unique(negSampleWordIndices, return_counts=True)
    unisampleidx = np.insert(unisampleidx, 0, outsideWordIdx)
    # To make sure the Uo is always the first row of score
    unicount = np.insert(unicount, 0, 1)
    score_vec = -1 * (
        outsideVectors[unisampleidx, :] @ centerWordVec
    )  # (N_unique_sample + 1) x 1
    score_vec[0] *= -1
    score_vec = sigmoid(score_vec)
    loss = -1 * unicount.dot(np.log(score_vec))

    score_vec = 1 / score_vec * score_vec * (1 - score_vec)
    sample_word_vec = outsideVectors[unisampleidx, :]
    sample_word_vec[0, :] *= -1
    gradCenterVec = unicount * score_vec @ sample_word_vec
    outside_addon = (
        unicount[:, np.newaxis]
        * score_vec[:, np.newaxis]
        @ centerWordVec[np.newaxis, :]
    )  # (N_unique_sample + 1) x D
    outside_addon[0, :] *= -1
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[unisampleidx, :] += outside_addon
...
def skipgram(
    ...
    cur_cw_vec = centerWordVectors[word2Ind[currentCenterWord]]
    for ow in outsideWords:
        cur_ow_idx = word2Ind[ow]
        cur_loss, cur_gradcenter, cur_gradout = word2vecLossAndGradient(
            cur_cw_vec, cur_ow_idx, outsideVectors, dataset
        )
        loss += cur_loss
        gradCenterVecs[word2Ind[currentCenterWord]] += cur_gradcenter
        gradOutsideVectors += cur_gradout
