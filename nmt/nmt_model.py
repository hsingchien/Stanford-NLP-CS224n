...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(
            input_size=embed_size + hidden_size, hidden_size=hidden_size
        )
    ...
    def encode(
        ...
        enc_hiddens, enc_lengths = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.transpose(enc_hiddens, 0, 1) #(b, src_len, h*2)
    ...
    def step(
        ...
        e_t = torch.bmm(enc_hiddens_proj,dec_hidden.unsqueeze(-1)) # enc_hiddens_proj (b, src_len, h), dec_hidden (b,h)
        # e_t (b,src_len,1)
        e_t = e_t.squeeze(-1)
        # e_t (b,src_len)
        ###
