...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        emb = self.embeddings.index_select(0,w.flatten())
        x = emb.view(w.shape[0], w.shape[1], self.embeddings.shape[1])
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w) # N_batch x N_feat x D_embedding
        x = x.view(x.shape[0],-1) # flatten to N_batch x (N_feat x D_embedding)
        h1 = x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias
        h1 = self.dropout(h1)
        relu = nn.ReLU()
        h1r = relu(h1)
        logits = h1r @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
