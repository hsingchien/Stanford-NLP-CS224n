...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        B, T, C = x.size()
        # print(f"T{T} C{C}")
        # B = batch, T = input length, C = embedding size
...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        att = self.w1(x) #(B*T,C)
        att = F.relu(att)
        att = att.view(B,T,self.n_head,C//self.n_head).transpose(1,2) # (B,nh,T,hs)
        att = att@self.w2[:,:T] + self.b2[:T] # w2 (hs, blocksize-1), att@w2 -> (B,nh,T,blocksize-1)
        att = att.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10)
        
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        y = att@v # att (B,nh,T,T), v(B,nh,T,hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side
