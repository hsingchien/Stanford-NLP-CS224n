...
argp.add_argument(
    "pretrain_corpus_path", help="Path of the corpus to pretrain on", default=None
)
argp.add_argument(
    "--reading_params_path",
...
argp.add_argument(
    "--finetune_corpus_path", help="Path of the corpus to finetune on", default=None
)
argp.add_argument(
    "--eval_corpus_path", help="Path of the corpus to evaluate on", default=None
)
...
if args.variant == "vanilla":
    setattr(mconf,"attention","vanilla")
elif args.variant == "synthesizer":
    setattr(mconf,"attention","synthesizer")
...
if args.function == "pretrain":
    ...
    pt_max_episode = 650
    pretrain_tconf = trainer.TrainerConfig(
        max_epochs=pt_max_episode,
        batch_size=128,
        learning_rate=6e-3,
        lr_decay=True,
        warmup_tokens=512 * 20,
    ...
    pretrain_trainer = trainer.Trainer(gptmodel, pretrain_dataset, None, pretrain_tconf)
    ...
    ft_max_episode = 10 if args.reading_params_path else 75
    if args.reading_params_path:
        gptmodel.load_state_dict(torch.load(args.reading_params_path))
    finetune_tconf = trainer.TrainerConfig(
        max_epochs=ft_max_episode,
        batch_size=256,
        learning_rate=6e-4,
        lr_decay=True,
        warmup_tokens=512 * 20,
    ...
    finetune_trainer = trainer.Trainer(gptmodel, finetune_dataset, None, finetune_tconf)
    ...
    gptmodel.load_state_dict(torch.load(args.reading_params_path,map_location="cuda:0"))
    gptmodel.to(device)
    correct = 0
    total = 0
