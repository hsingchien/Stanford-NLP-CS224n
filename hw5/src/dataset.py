...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #1 randomly truncate
        assert int(self.block_size*7/8)>=4
        tl = random.randint(4, int(self.block_size*7/8))
        # tl = min(tl, len(data_item))
        # start_idx = random.randint(0,len(data_item)-tl)
        # ts = data_item[start_idx:start_idx+tl]
        ts = data_item[:tl]
        #2 break into substrings
        while True:
            ml = int(random.gauss(int(1/4*tl),1))
            if ml>0 and ml < tl:
                break
        start_idx = random.randint(0, tl-ml)
        prefix = ts[:start_idx]
        masked_content = ts[start_idx:start_idx+ml]
        suffix = ts[start_idx+ml:]
        #3 rearrange the substrings
        masked_string = prefix+self.MASK_CHAR+suffix+\
            self.MASK_CHAR+masked_content+\
                self.PAD_CHAR*(self.block_size-len(self.MASK_CHAR)*2\
                    -len(prefix)-len(suffix)-len(masked_content))
        assert len(masked_string)==self.block_size
        #4 get x and y
        x = masked_string[:-1]
        y = masked_string[1:]
        #5 convert to long tensors
        xt = torch.tensor([self.stoi[xs] for xs in x], dtype=torch.long)
        yt = torch.tensor([self.stoi[ys] for ys in y], dtype=torch.long)
        return (xt, yt)
